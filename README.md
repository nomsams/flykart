...
- **Ruthless progress watchdog** evaluates candidates in configurable time windows and eliminates them after the chosen number of sub-threshold windows. Its minimum-progress threshold and patience are adjustable; it applies to training evaluation, not ordinary races.
...
- **Breeding priority** is a draggable progress-versus-reward slider with 10-point ratios (100/0, 90/10, 80/20, and so on). Its progress side can rank either total route coverage (“who got furthest”) or progress per second (“who advanced fastest”). Both the selected progress metric and reward are normalized within the generation before mating candidates are ranked. The separate winner-mating-share control decides how often the protected incumbent mates with the blended winner versus the runner-up.
...
If lap progress stalls, mutation rate and mutation magnitude rise above their defaults; after the configurable plateau patience window, a search burst adds random immigrants and larger scout mutations. Headless candidates share the same deterministic world seed within a run, so candidates are compared fairly instead of receiving different obstacle layouts; ghost candidates instead receive isolated copies of the world. Ghost contact is the default for simultaneous visual evolution, so candidates can sense and penalize traffic without rigid-body pileups; rigid collisions remain available by turning it off. Steering commands pass through a short actuator-response filter to remove frame-to-frame neural jitter. Checkpoint and finish gates span the full road width, use a locally blended tangent at sharp polygon corners, and accept a directed line crossing or a continuous forward arc crossing, so a car following the road cannot miss a corner gate. Finished race bots clear the racing lane into a finish area instead of remaining stacked on the line. Outside-track time contributes a per-second penalty in the same physics path used by racing and training, and speed is reduced to about half immediately outside the road with stronger drag farther away. Each car receives reward from local track progress, heading alignment, centerline position, ordered checkpoint gates, traffic proximity, hazard contact, and collision dynamics. A forward crossing of the actual start/finish boundary completes a lap only after the ordered gates and nearly the full route have been covered. Reaching the tick limit is reported as a time limit, not a crash.
