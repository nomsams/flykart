...
- **Evolution trend plot** shows incumbent reward, candidate reward, and forward progress over generations with separate fitness and route-coverage axes. Red shading marks plateau/exploration periods; generation points expose exact reward, progress, plateau, and mutation values on hover. The summary reports reward delta, progress delta, plateau streak, and mutation values. A plateau means no new incumbent for the configured patience; forward-progress gain below 2% is also treated as stalled.
