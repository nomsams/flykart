...
- **Adaptive time limit** can extend an improving candidate up to six times (configurable in the UI) using linear windows: 1,500 → 3,000 → 4,500 → 6,000 ticks by default. The first extension needs a non-negative recent reward rate; later extensions require improvement over the previous reward window.
...
- **Breeding priority** is a draggable progress-versus-reward slider with 10-point ratios (100/0, 90/10, 80/20, and so on). It defaults to 100% route progress and ranks the generation's actual mating candidates after normalizing reward to the current population, while completed laps and incumbent replacement remain progress-first. The separate winner-mating-share control still decides how often the protected incumbent mates with the blended winner versus the runner-up.
